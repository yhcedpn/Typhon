...
| `Targeting` | `long TargetRawEntityId` | packed raw ID（高位 `EntityKey`，低位 archetype ID），0 表示无锁。 |
...
当前公开 API 不能从 spatial raw `long` 恢复 `EntityId`，也没有公开的 `GetCluster(chunkId)`。`SpaceBattleFrameStore` 每帧按 `EntityKey` 重建并管理派生快照：

- `_publishedFrames`：本帧后续 Behavior/Damage/Movement 共享的只读逻辑快照；
- `_observedFrames`：合并本帧阶段更新、供 Observe 统计使用的镜像；
- generation 数组：只接受当前帧 Publish 过的槽，避免 entity destroy/migration 后出现陈旧值；
- `_entityIds`：把快照 `EntityKey` 映射回 Reap 所需的真实 `EntityId`。

Publish 之后，发布快照保持只读；阶段更新只写观察镜像。下一帧 `BeginTick` 推进 generation 并完整重建。该内存换 API 的决定见 [ADR-0003](file:///C:/Users/zyhs/source/repos/Typhon/docs/adr/0003-derived-entity-key-snapshots.md)。

### 2.3 旁路并发缓冲

不把跨实体写伪装成系统组件写。`SpaceBattleSimulationState` 只组合以下 internal deep modules：

- `SpaceBattleBehaviorModes`：行为模式推进、锁定转换，以及 Behavior/Movement 共同使用的航向和移动规则；
- `SpaceBattleCombatSettlement`：每个 worker 的 incoming-damage lane、touched-key 列表和 pending-reap buffer；
- `SpaceBattleAcquisitionTransactions`：每个 worker 的 thread-affine acquisition `Transaction` slot；
- `SpaceBattleTelemetryState`：targeting、combat、system/fence 的 worker-private 或受保护统计；
- `SpaceBattleFrameStore`：组件事实之外的本帧派生快照。
...
`BehaviorMode` 是 `Wandering`、`Tracking`、`Approaching`、`Attacking`、`Turning`；每个模式使用 `BehaviorPhase.Ready`、`Aligning`、`Flying` 中的一部分。`SpaceBattleBehaviorModes` 是状态推进和航向语义的唯一 module，Behavior 与 Movement system 只是不同阶段的 adapters。模式切换把 `ModeStartedTick` 写为下一逻辑帧编号，使随机派生与 worker/chunk 执行顺序无关；未知模式直接终止当前 tick，不会被归入 Wandering。
...
- **FramePrepare（Publish phase）**：每个 runtime tick 只推进派生快照 generation、attach `PointInTimeAccessor` 并记录 tick start。
- **Publish**：按 active cluster occupancy mask 读取五个组件，写入派生快照；bootstrap 首个 fence 的零填充异常窗口按 bootstrap 存活事实处理。只读组件，不创建 transaction。
- **Behavior**：按 cluster span 读发布快照，写当前遍历实体的 `Motion/Targeting/Behavior`；Tracking 使用 acquisition transaction 和 hybrid query；最终 tick 由实际使用 slot 的 worker 释放自己的 transaction。
...
- **Observe**：在 Reap 后发布 `SimulationTickCompleted`，每 `SpaceBattleTelemetrySampling.SamplePeriodTicks` 个 zero-based tick 生成稳定 telemetry sample，并更新完成 tick/剩余数。
...
所有核心系统是 Critical、`CanShed(false)`；Publish/Behavior/Damage/Movement 以固定 `workerCount × 2` chunk 分片，Reap/Observe 等串行尾部明确设为单 chunk。active cluster 数每 tick 重新读取，不能缓存旧 cluster 上界。四个宽系统通过零分配 `SpaceBattleOccupiedSlots` ref-struct 枚举 occupancy mask；不会在热循环引入 delegate、iterator 分配或间接调用。

## 5. 并发不变量

1. **单槽所有权**：一个 chunk 只由一个 worker 处理，worker 只能写当前遍历的实体槽；系统访问声明与 DAG 保证阶段之间的写读顺序。
2. **组件事实与派生快照分离**：组件是持久事实；Publish 后 `SpaceBattleFrameStore` 的发布快照保持只读，观察镜像只通过受控更新方法合并本 tick 的阶段结果。
...
7. **transaction thread affinity**：acquisition transaction 只能在创建它的 worker 线程复用/释放；runtime 停止后 Host 才调用 `ReleaseAllAfterRuntimeStop` 做 best-effort 清理，不能在 runtime 存活时跨线程释放。
...
相同 seed + 相同固定 worker 数的 deterministic diagnostic 比较 health/target/mode checksum；固定 worker Host 在配置的最大逻辑帧后让后续已调度系统成为 no-op，消除停止竞态导致的额外状态推进。跨 worker 数比较始终标为 Explicit，不宣称 worker-count-independent determinism。
...
当前 Typhon 没有公开轻量 epoch scope，而 cluster spatial query 需要 epoch；`EpochGuard` 是 internal。因此 demo 按 [ADR-0007](file:///C:/Users/zyhs/source/repos/Typhon/docs/adr/0007-per-worker-acquisition-transactions.md) 使用 `SpaceBattleAcquisitionTransactions`：

- worker 第一次进入 Behavior 时按需创建一个 read-only transaction；
- 同一逻辑帧的多个 Behavior chunk 复用同一 slot，避免每 chunk 并发创建 transaction 的锁争用；
- transaction 是 thread-affine，只由创建它的 worker 使用和释放；
- slot 通常允许跨一个 tick fence 延用：创建 tick `t` 可在下一帧继续使用，worker 再下一次进入 Behavior 时释放并替换；
- 最终 tick 由 Behavior 中实际使用 slot 的 worker 在自身线程释放；Host 停止 runtime 后再做 best-effort 全量清理；
- 固定 worker 的 determinism diagnostic 每 tick 只递增失效版本；worker 下一次使用自己的 slot 时在所有者线程释放并替换，避免跨线程 `Dispose`；
- Publish、Damage和Movement不创建 acquisition transaction。
...
应用侧 `TickTiming` 和 system accumulator 只保留最近 4,096 个 duration 样本，并以有界复制/排序成本计算 p50/p95/p99/max；`>40 ms` 计数严格不包含 40 ms，也只覆盖该窗口。`SpaceBattleTelemetrySampling` 在 zero-based tick 0 和每 `SamplePeriodTicks`（当前为125）tick采样，字段使用 invariant-culture。系统 accumulator 分解应用阶段和 fence 阶段；系统列表只存在于 `SpaceBattleTelemetrySnapshot.Systems`，不在 `TickPerformanceSnapshot` 保存副本；`StragglerGapUs` 不输出，因为当前是占位值 0。
...
Ctrl+C 只设置 cancellation；Host 记录请求时的 completed/runtime tick，通过逻辑帧完成信号等待 in-flight tick 到边界，再 `Shutdown()`，避免 Sleep/SpinWait 轮询和多跑后续 tick。若同时观测到系统异常，优先 `FatalStop()` 并返回 `fatal`。
...
- 输出 bootstrap、termination/remaining、稳定 telemetry 和最新 sample 的系统/fence 分解。系统 accumulator 是截至该 sample 的最近 4,096 个 duration 样本，所以只把它作为实测成本分解，不把它误称为 warmup-trimmed phase 百分位，也不保证早期 warmup 仍在窗口内；runtime 没有有效样本的 fence 条目输出 0 并标注 `unexposed_zero_not_cost`。p95 >40 ms 只输出 warning，不是跨机器 CI 硬门槛；
