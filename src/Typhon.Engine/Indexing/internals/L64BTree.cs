...
unsafe public struct Index64Chunk
{
    ...
    public long HighKey;                            // B-link upper bound — co-located with OlcVersion in first 128B region
    public fixed int Values[Capacity];              // 19 × 4 = 76 bytes
    public fixed long Keys[Capacity];               // 19 × 8 = 152 bytes
...
internal abstract class L64BTree<TKey, TStore> : BTree<TKey, TStore> where TStore : struct, IPageStore where TKey : unmanaged
{
    unsafe public class L64NodeStorage : BaseNodeStorage
    {
        internal override void Initialize(BTree<TKey, TStore> owner, ChunkBasedSegment<TStore> segment)
        {
            ...
            Debug.Assert(sizeof(Index64Chunk) == 256);
        ...
        public override void InitializeNode(NodeWrapper node, NodeStates states, ref ChunkAccessor<TStore> accessor)
        {
            ...
            chunk.NextChunk = 0;
            chunk.LeftValue = 0;
            chunk.HighKey = long.MaxValue; // B-link upper bound: sentinel for newly allocated nodes
        }
        ...
        {
            ref var chunk = ref accessor.GetChunk<Index64Chunk>(chunkId, false);
